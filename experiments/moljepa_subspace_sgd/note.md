...
The experiment contains no production edits. `probe.py` takes `--checkpoint` and `--source-file` paths and checks pinned hashes before source execution. New runs default to `replay-results.json`; the archived evidence remains in `results.json`. Dependencies are PyTorch, PyG, safetensors and the original featurizer dependencies molfeat/RDKit. No novelty claim is made; the related linear-subspace/Gram-SGD note from the previous experiment provides primary-source context.
