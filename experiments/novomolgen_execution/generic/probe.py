...
def tensor_byte_comparison(left, right):
    """Compare each tensor's value bytes, including signed zeros, independently.

    Shape and dtype must also match. Copies preserve dtype and compare CPU
    contiguous logical values, not backing-storage padding or alias identities.
    Python metadata/structure is checked; every floating, integer and boolean
    tensor leaf is counted. This does not rely on compare_outputs()['exact'].
    """
    result = {"method": "CPU contiguous reshape(-1).view(torch.uint8)",
              "tensor_leaves": 0, "bytes_checked": 0, "tensor_leaves_by_dtype": {},
              "failures": []}
    def walk(a, b, path):
        if isinstance(a, torch.Tensor) or isinstance(b, torch.Tensor):
            result["tensor_leaves"] += 1
            if not isinstance(a, torch.Tensor) or not isinstance(b, torch.Tensor):
                result["failures"].append({"path": path, "kind": "tensor_type_mismatch"})
                return
            dtype = str(a.dtype)
            result["tensor_leaves_by_dtype"][dtype] = result["tensor_leaves_by_dtype"].get(dtype, 0) + 1
            if a.shape != b.shape or a.dtype != b.dtype:
                result["failures"].append({"path": path, "kind": "shape_or_dtype_mismatch",
            ...
            b_bytes = b.detach().cpu().contiguous().reshape(-1).view(torch.uint8)
            result["bytes_checked"] += a_bytes.numel()
            if not torch.equal(a_bytes, b_bytes):
                result["failures"].append({"path": path, "kind": "different_tensor_bytes",
                    "shape": list(a.shape), "dtype": dtype,
                    "different_bytes": int(torch.count_nonzero(a_bytes != b_bytes))})
        elif isinstance(a, Mapping) or isinstance(b, Mapping):
            if not isinstance(a, Mapping) or not isinstance(b, Mapping) or list(a) != list(b):
                result["failures"].append({"path": path, "kind": "mapping_structure_mismatch"})
                return
            for key in a:
                walk(a[key], b[key], f"{path}[{key!r}]")
        elif isinstance(a, (tuple, list)) or isinstance(b, (tuple, list)):
            if type(a) is not type(b) or len(a) != len(b):
                result["failures"].append({"path": path, "kind": "sequence_structure_mismatch"})
                return
            for index, (x, y) in enumerate(zip(a, b)):
                walk(x, y, f"{path}[{index}]")
        elif type(a) is not type(b) or a != b:
            result["failures"].append({"path": path, "kind": "metadata_mismatch"})
...
def main():
    ...
    parser.add_argument("--lookup-artifact", type=Path)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    torch.set_num_threads(4)
    reference = load_reference(args.checkpoint, args.config, device=args.device, backend=args.backend)
    candidate = make_candidate(reference, table_device=args.table_device, lookup_artifact=args.lookup_artifact)
    records = []
    ...
    def record(name, left, right):
        try:
            ...
            records.append({"case": name, "accepted": not failed and byte_comparison["accepted"],
                "numeric_accepted": not failed, "tensor_byte_comparison": byte_comparison, "tensor_outputs": len(metrics),
    ...
    result = {"method": "first_layer_finite_qkv_research", "device": args.device,
        ...
        "table_policy": "General compile_finite_fanout with explicit backend profiles and full native token-only context",
        "lookup_recipe": candidate._first_qkv_lookup.recipe(),
        "lookup_compilation_report": candidate._first_qkv_compilation_report,
        "lookup_artifact_reloaded": args.lookup_artifact is not None,
        "lookup_stored_values": sum(p.numel() for p in candidate._first_qkv_lookup.parameters()),
        "native_embedding_retained_values": candidate.model.embed_tokens.weight.numel(),
        ...
        "all_tensor_bytes_equal": all(row.get("tensor_byte_comparison", {}).get("accepted", False) for row in records),
        "byte_tensor_leaves": sum(row.get("tensor_byte_comparison", {}).get("tensor_leaves", 0) for row in records),
        "bytes_checked": sum(row.get("tensor_byte_comparison", {}).get("bytes_checked", 0) for row in records),
