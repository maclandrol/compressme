...
class TokenLookupCausalLM(LlamaForCausalLM):
    def forward(self, input_ids=None, attention_mask=None, position_ids=None,
        ...
        if input_ids.device.type == 'mps' and 1 < input_ids.numel() < 16:
            rows = self._first_qkv_small_rows
...
def make_candidate(source, *, table_device="cpu"):
    ...
    widths, tables, singleton_tables, small_tables = [], [], [], []
    with torch.inference_mode(False), torch.no_grad():
        ...
        for name in ("q_proj", "k_proj", "v_proj"):
            ...
            small_tables.append(torch.cat([
                projection(encoder(embeddings[token].reshape(1, 1, -1).expand(1, 2, -1).clone()))[:, 0, :]
                for token in range(embeddings.shape[0])
            ], dim=0).to(source_device))
        rows = torch.cat(tables, dim=-1).clone()
        singleton_rows = torch.cat(singleton_tables, dim=-1).clone()
        small_rows = torch.cat(small_tables, dim=-1).clone()
    ...
    candidate._first_qkv_small_rows = nn.Parameter(small_rows, requires_grad=False)
