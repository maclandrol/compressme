...
from prototype import load_reference, make_candidate, _CURRENT
...
def observable(value):
    if isinstance(value, DynamicCache):
        return {"cache_type": type(value).__name__, "sequence_length": value.get_seq_length(),
                "layers": observable(value.to_legacy_cache())}
...
def main():
    ...
    with torch.inference_mode():
        ...
        for mode in ("legacy", "dynamic"):
            caches = [None, None] if mode == "legacy" else [DynamicCache(), DynamicCache()]
            for step in range(4):
                step_ids = ids if step == 0 else torch.full((2, 1), 8+step, device=device)
                outputs = []
                ...
                record(f"{mode}_cache_step_{step}", *outputs)
        tokenizer = AutoTokenizer.from_pretrained(Path(args.config).parent, local_files_only=True)
        prompts = ["C", "CCO", "c1ccccc1"]
        inputs = tokenizer(prompts, padding=True, return_tensors="pt", return_token_type_ids=False).to(device)
        for name, options in [("greedy", {"do_sample": False}),
            ...
            def generation_observable(output):
                ...
                return result
            record(f"generation_{name}", *(generation_observable(x) for x in outputs))
