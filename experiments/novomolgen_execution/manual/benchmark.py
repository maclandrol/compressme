...
sys.path.insert(0,str(Path(__file__).parent/('singleton' if args.device=='cpu' else 'regimes')))
...
result={'scope':'Same-backend, same-cache token-ID native model API latency; input transfer and tokenizer excluded',
    'device':args.device,'backend':'eager','python':platform.python_version(),'torch':torch.__version__,
    'transformers':transformers.__version__,'cpu_threads':torch.get_num_threads(),'warmup_pairs':3,
