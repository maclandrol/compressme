    ...
    def activate(self):
        ...
        try:
            ...
            for owner,name,original in reversed(installed):
                try:setattr(owner,name,original)
        ...
        try:
            ...
            try:
                ...
                if 'forward' in score.__dict__:
                    del score.forward
                memo.clear()
                report['memo_cleared'] = True
                report['cache_cleared'] = restoration_error is None
            ...
            try:
                ...
                if hasattr(primary_error, 'add_note'):
                    primary_error.add_note('Sampling cleanup also failed: ' + type(cleanup_error).__name__)
    diffusion.sample = sample_with_constants
    try:
        yield statistics
    finally:
        if 'sample' in diffusion.__dict__:del diffusion.sample
        if 'forward' in score.__dict__:
            del score.forward
