...
def check_layout(x,expected,amp=None):
    if torch.is_grad_enabled():raise ValueError('Prepared forward requires no-grad or inference execution')
    if signature(x)!=expected:raise ValueError('Static input layout changed')
    if amp is not None and autocast_state(x)!=amp:raise ValueError('Autocast state changed')

class PreparedAdaLN(nn.Module):
    def __init__(self,source,s):
        super().__init__();self.a_norm=source.a_norm;self.expected=signature(s);self.amp=autocast_state(s)
    ...
    def forward(self,a,s):
        check_layout(s,self.expected,self.amp)
        return self.scale*self.a_norm(a)+self.bias

class PreparedOutputGate(nn.Module):
    def __init__(self,source,s):
        super().__init__();self.expected=signature(s);self.amp=autocast_state(s)
        self.register_buffer('value',source(s),persistent=False);self.eval()
    def forward(self,s):
        check_layout(s,self.expected,self.amp);return self.value
...
class PreparedSingleConditioningPrefix(nn.Module):
    ...
    def __init__(self,source,s_trunk,s_inputs):
        ...
        self.trunk_signature=signature(s_trunk);self.input_signature=signature(s_inputs);self.amp=autocast_state(s_trunk)
    ...
    def forward(self,times,s_trunk,s_inputs):
        check_layout(s_trunk,self.trunk_signature,self.amp);check_layout(s_inputs,self.input_signature,self.amp)
        s=self.prefix
...
def producer_signature(score):
    paths=['single_conditioner.norm_single','single_conditioner.single_embed']
    for stem in ['atom_attention_encoder.atom_encoder','atom_attention_decoder.atom_decoder']:
        block=score.get_submodule(stem).diffusion_transformer
        for i in range(len(block.layers)):
            base=f'{stem}.diffusion_transformer.layers.{i}.'
            paths.extend(base+tail for tail in ['adaln','transition.adaln','output_projection','transition.output_projection'])
    result=[]
    for path in paths:
        producer=score.get_submodule(path);records=[]
        for name,module in producer.named_modules():
            config=(getattr(module,'eps',None),getattr(module,'normalized_shape',None))
            bindings=tuple((key,state_token(value) if value is not None else None) for key,value in list(module._parameters.items())+list(module._buffers.items()))
            records.append((name,id(module),module.training,id(module.__dict__.get('forward')),config,bindings))
        result.append((path,tuple(records)))
    return tuple(result)
...
def constant_conditioning_sampling_scope(model):
    ...
    Producer bindings, layouts, storage metadata and tracked Tensor versions are checked before use without reading GPU data;
    a detected static-input/state change falls back to the original score call. Unsafe .data in-place writes can bypass version counters and remain outside the declared immutable-request contract. Autocast-enabled score calls use the original path.
    """
    ...
    def sample_with_constants(*sample_args,**sample_kwargs):
        ...
        input_versions=[state_token(t) for t in (static_c,static_trunk,static_inputs)] if eligible else []
        parameter_versions=producer_signature(score)
        def score_with_constants(*score_args,**score_kwargs):
            ...
            if same:
                same=not torch.is_grad_enabled() and not autocast_state(static_c)[0] and score.single_conditioner is original_single
                same=same and input_versions==[state_token(t) for t in (static_c,static_trunk,static_inputs)] and parameter_versions==producer_signature(score)
            if not same:
