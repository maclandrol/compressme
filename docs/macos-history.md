...
These experiments were recorded on 2026-09-07 using an Apple M5 MacBook Air with 16 GB unified memory, macOS 26.6.1, arm64, Python 3.12.14 and PyTorch 2.14.0. The first experiment tests constructed factor matrices, not compressed Boltz-2 or Mol-JEPA weights. Later sections cover the first trained Mol-JEPA rewrites. For the supported runtime and later results, see [macos.md](macos.md) and [runtime.md](runtime.md).
...
Installed inference dependencies include torch 2.14.0, numpy 2.5.3, transformers 5.16.1, torch-geometric 2.8.0.post1, rdkit 2026.3.6, molfeat 0.11.0, safetensors 0.8.0. The commands record the environment setup used for these experiments; reproducing it requires pinning the listed versions.

The restricted execution sandbox reported `torch.backends.mps.is_available() == False`. The same interpreter with approved host execution access reported `True`, and both forward and backward matrix operations ran successfully on the actual Metal GPU. MPS availability depended on execution permissions in this environment.

## Synthetic matrix benchmark

Weights were constructed as `W = B @ A`, with random float32 factors. Dense execution was `linear(x, W, bias)` and factored execution was `linear(linear(x, A), B, bias)`. There is no low-rank truncation in this experiment. Floating-point reassociation explains the nonzero differences.
...
## Bilinear graph-attention contraction on the trained checkpoint
...
The bilinear score rewrite was the fastest variant on this tested MPS workload. Moving aggregation before value projection is algebraically valid for the audited conditions, but its runtime benefit is backend-dependent.
...
An additional exact simplification skipped score computation for receiving atoms with a single incoming neighbour, whose softmax attention is identically one. On the same four-SMILES input, a separate warmed 20-round interleaved comparison found **10.831 ms without skipping versus 13.727 ms with skipping**, approximately 27% slower. It remained disabled on this MPS workload because extra dynamic indexing outweighed arithmetic savings. Output errors remained at the same small numerical scale. These absolute timings must not be compared across runs; only their paired ratio is meaningful here. Evidence: `/private/tmp/compressme_bilinear_skip_interleaved_mps.json`.

## Early package verification

A persistent editable environment was created at `/Users/manu/Code/compressme/.venv` from the existing downloaded cache. It contains the core package and its `test,molecules` optional dependencies. Direct import resolves to the requested project source. At this stage, the complete package suite passed **31 tests in 6.47 seconds**, including actual Metal access.

Independent review found and verified fixes for: unsupported/incorrect combined Metal float64 transfers; complex weight truncation; frozen parameters becoming trainable; stale numerical bounds being revalidated after export; and mixed CPU/MPS restored models. Those regressions cover actual MPS low-rank, covariance, LayerNorm and bound-provenance cases.

## Backend and API lessons from these experiments

Ordinary PyTorch modules and dense factors ran on MPS while retaining the Python model code and supporting inference and autograd. MLX remained a possible later backend, requiring graph or source conversion because its module and array APIs differ. Its unified memory and lazy execution do not by themselves compress a model.
...
Select compressed execution only when `r*(input+output) < input*output`, and benchmark candidate ranks on representative shapes. Two matrix multiplies have an intermediate tensor and extra launch overhead. For the measured batch-one 768→3072 factored operation, CPU was faster than MPS.
...
Fast-math and CPU fallback settings affect fidelity or execution. PyTorch documents both; fallback moves unsupported operations to CPU, so whole-model timings must include that work and record the setting.
